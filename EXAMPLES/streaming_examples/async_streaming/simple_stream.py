...
async def stock_trades_handler(
    msg,
...
async def main():
    ...
    await stream_client.subscribe_stock_trades(
        handler_function=stock_trades_handler
    )  # ALL tickers
    await stream_client.subscribe_stock_second_aggregates(
        ["AMD", "NVDA"], stock_aggregates_handler
    )
