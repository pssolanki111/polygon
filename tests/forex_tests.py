...
class TestForex(unittest.TestCase):
    def test_get_historic_forex_ticks(self):
        with polygon.ForexClient(cred.KEY) as client:
            data = client.get_historic_forex_ticks(
                "AUD", "USD", "2021-10-15", limit=5)
            data2 = client.get_historic_forex_ticks(
                "AUD", "USD", datetime.date(2021, 10, 15), limit=5, raw_response=True
            )
    ...
    def test_get_quotes(self):
        with polygon.ForexClient(cred.KEY) as client:
            ...
            data3 = client.get_quotes(
                "C:EUR-USD", limit=5, all_pages=True, max_pages=2)
            data4 = client.get_quotes(
                "C:EUR-USD", limit=5, all_pages=True, max_pages=2, merge_all_pages=False
            )
    ...
    def test_get_aggregate_bars(self):
        with polygon.ForexClient(cred.KEY) as client:
            data = client.get_aggregate_bars(
                "C:EURUSD", "2021-09-10", datetime.date(2021, 10, 1), limit=30
            )
    ...
    def test_real_time_currency_conversion(self):
        with polygon.ForexClient(cred.KEY) as client:
            data = client.real_time_currency_conversion(
                "AUD", "USD", amount=10)
            data2 = client.real_time_currency_conversion(
                "USD", "AUD", amount=10, raw_response=True
            )
    ...
    async def test_async_get_historic_forex_ticks(self):
        async with polygon.ForexClient(cred.KEY, True) as client:
            data = await client.get_historic_forex_ticks(
                "AUD", "USD", "2021-10-15", limit=5
            )
            data2 = await client.get_historic_forex_ticks(
                "AUD", "USD", datetime.date(2021, 10, 15), limit=5, raw_response=True
            )
    ...
    async def test_async_real_time_currency_conversion(self):
        async with polygon.ForexClient(cred.KEY, True) as client:
            ...
            data2 = await client.real_time_currency_conversion(
                "USD", "AUD", amount=10, raw_response=True
            )
