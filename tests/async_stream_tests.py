...
class TestStocksStream(unittest.TestCase):
    ...
    async def test_streaming_stocks(self):
        ...
        await streamer.subscribe_stock_trades(
            ["AMD", "NVDA", "TSLA", "MSFT", "GOOG"], self.message_handler
        )
        ...
        await streamer.subscribe_stock_quotes(
            ["AMD", "NVDA", "TSLA", "MSFT", "GOOG"],
        ...
        await streamer.subscribe_stock_second_aggregates(
            ["AMD", "NVDA", "TSLA", "MSFT", "GOOG"],
            handler_function=self.message_handler,
        )
        ...
        await streamer.unsubscribe_stock_second_aggregates(
            ["AMD", "NVDA", "TSLA", "MSFT", "GOOG"]
        )
...
class TestOptionsStream(unittest.TestCase):
    ...
    async def test_streaming_options(self):
        ...
        await streamer.subscribe_option_second_aggregates(
            handler_function=self.message_handler
        )
...
class TestForexStream(unittest.TestCase):
    ...
    async def test_streaming_forex(self):
        ...
        await streamer.subscribe_forex_quotes(
            ["EUR/USD", "C:USD/EUR", "AUD/USD"], handler_function=self.message_handler
        )
        asyncio.ensure_future(self.handle_messages(streamer))
...
class TestCryptoStream(unittest.TestCase):
    ...
    async def test_streaming_crypto(self):
        ...
        await streamer.subscribe_crypto_trades(
            ["BTC-USD", "X:LTC-USD"], handler_function=self.message_handler
        )
        asyncio.ensure_future(self.handle_messages(streamer))
        ...
        await streamer.subscribe_crypto_quotes(
            ["BTC-USD", "X:LTC-USD"], handler_function=self.message_handler
        )
        asyncio.ensure_future(self.handle_messages(streamer))
        ...
        await streamer.subscribe_crypto_level2_book(
            ["BTC-USD", "X:LTC-USD"], handler_function=self.message_handler
        )
        asyncio.ensure_future(self.handle_messages(streamer))
